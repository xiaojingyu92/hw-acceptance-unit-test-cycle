  ...
  describe '.find_similar_movies' do
    ...
    context 'director exists' do
      it 'finds similar movies correctly' do
        expect(Movie.same_director(movie1.title)).to eql(['Catch me if you can', "Schindler's List"])
        expect(Movie.same_director(movie1.title)).to_not include(['Seven'])
        expect(Movie.same_director(movie2.title)).to eql(['Seven'])
      end
    end
